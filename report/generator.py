...
def print_report(keyword: str, results: dict) -> None:
    ...
    color_map = {"green": "bright_green", "yellow": "yellow", "orange": "dark_orange", "red": "red"}
    sc_color = color_map.get(sc.get("color", ""), "white")
    console.print(Panel(
        f"[bold {sc_color}]{score_val} / 100[/bold {sc_color}]  —  {verdict}",
        title="Indice de Oportunidad",
        box=box.ROUNDED,
    ))
    ...
    # ─── Ideas de contenido ───────────────────────────────────────────────────
    _section("Ideas de contenido para esta semana", "bold cyan")
    ideas = results.get("content_ideas", [])
    for i, idea in enumerate(ideas, 1):
        console.print(f"  [bold]{i}.[/bold] {idea}")
